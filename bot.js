...
const CLIENT_ID = process.env.DISCORD_CLIENT_ID || "1536312317300441089";
const GUILD_ID = process.env.DISCORD_GUILD_ID || "1524506931165659239";
...
const commands = [
    ...
    .addSubcommand(sub =>
      sub
        ...
        .addStringOption(opt =>
          opt
            .setName("duracion")
            .setDescription("Duracion de la key")
            .setRequired(true)
            .addChoices(
  ...
  new SlashCommandBuilder()
    .setName("stats")
    .setDescription("Mostrar estadisticas"),
  ...
  new SlashCommandBuilder()
    .setName("ping")
    .setDescription("Comprobar estado")
...
async function getAdminToken() {
  ...
  if (!response.ok || !data.success || !data.token) {
    throw new Error(
      data.message || "No se pudo iniciar sesion en la API"
    );
  }
...
async function adminApiRequest(endpoint, options = {}) {
  ...
  if (!response.ok) {
    throw new Error(
      data.message || "API respondio " + response.status
    );
  }
...
client.once("ready", async () => {
  console.log("======================================");
  console.log("ROBLOX KEY DISCORD BOT");
  console.log("======================================");
...
client.on("interactionCreate", async interaction => {
  ...
  if (interaction.guildId !== GUILD_ID) {
    return interaction.reply({
      content: "Este bot no esta autorizado para este servidor.",
      ephemeral: true
    });
  }
  ...
  try {
    ...
    if (interaction.commandName === "help") {
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          "Sistema de administracion de keys conectado a Roblox."
        )
        .addFields(
          {
            name: "Keys",
    ...
    if (interaction.commandName === "key") {
      ...
      if (subcommand === "listar") {
        ...
        if (!data.keys || !data.keys.length) {
          return interaction.editReply({
            content: "No hay keys registradas."
          });
        }
    ...
    if (interaction.commandName === "whitelist") {
      ...
      if (subcommand === "listar") {
        ...
        if (!data.length) {
          return interaction.editReply({
            content: "La whitelist esta vacia."
          });
        }
    ...
    if (interaction.commandName === "stats") {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: "Revocadas",
            value: String(data.revokedKeys || 0),
            inline: true
          },
          {
            name: "Whitelist",
            value: String(data.whitelistUsers || 0),
            inline: true
          }
        )
        .setTimestamp();
